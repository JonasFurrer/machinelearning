...
def linear(x, y, b=1):
    ...
    result = np.dot(x,np.transpose(y))+b
...
def gaussian(x, y, sigma=1):    
    ...
    if np.ndim(x) == 1 and np.ndim(y) == 1:
        result = np.exp(- np.power(np.linalg.norm(x - y),2) / (2 * sigma ** 2))
    elif (np.ndim(x) > 1 and np.ndim(y) == 1) or (np.ndim(x) == 1 and np.ndim(y) > 1):
        result = np.exp(- np.power(np.linalg.norm(x - y, axis=1),2) / (2 * sigma ** 2))
    elif np.ndim(x) > 1 and np.ndim(y) > 1:
        result = np.exp(- np.power( np.linalg.norm(x[:, np.newaxis] - y[np.newaxis, :], axis=2),2) / (2 * sigma ** 2))
